...
export APIScriptVersion=v00x17
ScriptName=cli_api_export_object-names_to_csv_$APIScriptVersion
...
MainOperationalProcedure () {
    ...
    echo '"name"' > $APICLICSVfileheader
    
#    mgmt_cli show $APICLIobjecttype limit $APICLIObjectLimit offset 0 details-level "$APICLIdetaillvl" --format json -s $APICLIsessionfile | $JQ ".name" -r >> $APICLICSVfile.data
#    mgmt_cli show $APICLIobjecttype limit $APICLIObjectLimit offset 0 details-level full --format json -s $APICLIsessionfile | $JQ ".objects[].name | @sh" -r >> $APICLICSVfile.data
    mgmt_cli show $APICLIobjecttype limit $APICLIObjectLimit offset 0 details-level full --format json -s $APICLIsessionfile | $JQ ".objects[].name" -r >> $APICLICSVfile.data
