...
#echo '"name","color","comments","icon"' > $APICLICSVfileheader

#mgmt_cli show $APICLIobjecttype limit $APICLIObjectLimit offset 0 details-level "$APICLIdetaillvl" --format json -s $APICLIsessionfile | $JQ '.objects[] | [ .["name"], .["color"], .["comments"], .["icon"] ] | @csv' -r >> $APICLICSVfile.data
